...
(define (read-to-whitespace str)
  (let ((char (peek-char)))
    (if (whitespace? char) ; or delimiter ??
      (list->string (reverse str))
      (read-to-whitespace (cons (read-char) str)))
  )
)
...
(define (space? char)
  (and 
    (char=? char #\s) 
    (string=? (read-to-whitespace '()) "pace")
  )
)
...
(define (newline? char)
  (and
    (char=? char #\n)
    (string=? (read-to-whitespace '()) "ewline")
  )
)
